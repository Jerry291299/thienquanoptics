...
mongoose
  .connect(
    ...
    "mongodb+srv://thienquangoptics1:thienquangoptics1@cluster0.mxr2cbp.mongodb.net/",
    {
...
app.post("/product/add", async (req: Request, res: Response) => {
  try {
    // Destructure the request body, excluding `img` and including `gender`
    const { masp, name, moTa, brand, categoryID, gender, status, variants } = req.body;

    // **Input Validation**
    // Check for required fields
    if (!masp || !name || !moTa || !brand || !categoryID || !gender || typeof status !== "boolean" || !variants) {
    ...
    // Ensure each variant has a non-empty images array with string URLs
    for (const variant of variants) {
      if (!variant.images || !Array.isArray(variant.images) || variant.images.length === 0) {
        return res.status(400).json({ message: `Biến thể ${variant.color} phải có ít nhất một hình ảnh` });
      }
      if (!variant.images.every((img: any) => typeof img === "string")) {
        return res.status(400).json({ message: `Hình ảnh trong biến thể ${variant.color} phải là chuỗi URL` });
      }
    }
    ...
    const newProduct = new Product({
      ...
      status,
      variants, // Images are now part of variants
    });
...
app.put("/product/:id", async (req: Request, res: Response) => {
  try {
    ...
    if (gender && !["Male", "Female", "Kids"].includes(gender)) {
      return res.status(400).json({ message: "Giới tính không hợp lệ, phải là 'Male', 'Female', hoặc 'Kids'" });
    }
    ...
    if (variants) {
      ...
      // Validate images in each variant
      for (const variant of variants) {
        if (!variant.images || !Array.isArray(variant.images) || variant.images.length === 0) {
          return res.status(400).json({ message: `Biến thể ${variant.color} phải có ít nhất một hình ảnh` });
        }
